...
const KNOWN_ERRORS: [RegExp, string][] = [
  ...
  [/not a bot/i, "YouTube asked this server to prove it isn't a bot. Wait a few minutes, then try again."],
  ...
  [/http error 429|too many requests/i, "YouTube is rate-limiting this server. Wait a few minutes, then try again."],
...
export function explainYtDlpFailure(err: unknown): Error {
  ...
  for (const [pattern, message] of KNOWN_ERRORS) {
    if (pattern.test(errorLine)) return new UserFacingError(message);
  }
